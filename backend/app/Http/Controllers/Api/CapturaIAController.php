 ...
 * ver `empresaIdOrFail()`. `TenantScope` ya filtra `CapturaIA::query()` y
 * el route-model-binding de `{captura}` automáticamente; los
 * `authorize()` de abajo son la segunda capa de defensa explícita, por si
 * ese scope alguna vez se bypasea.
 */
class CapturaIAController extends Controller
{
    ...
    public function show(Request $request, CapturaIA $captura): JsonResponse
    {
    ...
    public function confirmar(Request $request, CapturaIA $captura): JsonResponse
    {
    ...
    public function descartar(Request $request, CapturaIA $captura): JsonResponse
    {
        $this->authorize('update', $captura);
