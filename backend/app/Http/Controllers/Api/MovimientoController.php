...
class MovimientoController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $movimientos = $query->latest()->paginate($this->perPageDeRequest($request, 100));

        return ApiResponse::success([
            'items' => MovimientoResource::collection($movimientos)->resolve(),
            'meta' => $this->metaDePaginacion($movimientos),
        ]);
    }

    public function show(int $movimiento): JsonResponse
    {
    ...
    public function store(StoreMovimientoRequest $request): JsonResponse
    {
        ...
        if (! empty($datos['proveedor_id'])) {
            $proveedorNombre = $this->resolverParaEmpresaActual(Proveedor::class, (int) $datos['proveedor_id'])->nombre;
        }
     ...
     * Categoria/Marca/UnidadMedida/StockController): `$movimiento->update()`
     * ya refleja los nuevos valores en la instancia en memoria — un
     * `->fresh()` posterior era un SELECT evitable.
     */
    public function update(UpdateMovimientoRequest $request, int $movimiento): JsonResponse
    {
        $movimiento = $this->resolverParaEmpresaActual(Movimiento::class, $movimiento);
