...
class ProductoProveedorController extends Controller
{
     ...
     */
    public function index(int $producto): JsonResponse
    {
    ...
    public function store(StoreProductoProveedorRequest $request, int $producto): JsonResponse
    {
        $producto = $this->resolverParaEmpresaActual(Producto::class, $producto);
        $this->authorize('update', $producto);
        $this->authorize('create', ProductoProveedor::class);

        // El proveedor debe pertenecer a la misma empresa — resuelto
        // explícitamente (ADR-019); si no aparece, es de otra empresa o
        // no existe, mismo 404 de siempre.
        $proveedor = $this->resolverParaEmpresaActual(Proveedor::class, (int) $request->validated('proveedor_id'));
    ...
    public function update(UpdateProductoProveedorRequest $request, int $producto, int $asociacion): JsonResponse
    {
        $producto = $this->resolverParaEmpresaActual(Producto::class, $producto);
        $asociacion = $this->resolverParaEmpresaActual(ProductoProveedor::class, $asociacion);
    ...
    public function disable(Request $request, int $producto, int $asociacion): JsonResponse
    {
        $producto = $this->resolverParaEmpresaActual(Producto::class, $producto);
        $asociacion = $this->resolverParaEmpresaActual(ProductoProveedor::class, $asociacion);
        ...
        return ApiResponse::success(
            new ProductoProveedorResource($asociacion),
