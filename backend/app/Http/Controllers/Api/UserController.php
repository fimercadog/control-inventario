 ...
 *
 * ADR-019: sin Global Scope automático en ningún modelo. `User` tampoco
 * usa `BelongsToEmpresa` (no es un recurso de catálogo) — cada método de
 * este controller filtra manualmente por `empresa_id` (`empresaIdActual()`)
 * antes de resolver un usuario por id, así que un id de otra empresa
 * siempre resulta en `ModelNotFoundException` (404), nunca en un 403 que
...
class UserController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $query = User::query()
            ->with('invitedBy')
            ->where('empresa_id', $this->empresaIdActual());
        ...
        $usuarios = $query->orderBy('name')->paginate($this->perPageDeRequest($request, 100));

        return ApiResponse::success([
            'items' => UserResource::collection($usuarios)->resolve(),
            'meta' => $this->metaDePaginacion($usuarios),
        ]);
    }
    ...
    {
        return User::where('empresa_id', $this->empresaIdActual())
            ->findOrFail($id);
    }
