...
class ClienteControllerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $rolA = Role::create(['name' => 'Test Clientes A', 'guard_name' => 'api', 'empresa_id' => $this->empresaA->id]);
        $rolA->givePermissionTo($permisos);
        $this->userA->assignRole($rolA);
        $registrar->forgetCachedPermissions();

        $registrar->setPermissionsTeamId($this->empresaB->id);
        $rolB = Role::create(['name' => 'Test Clientes B', 'guard_name' => 'api', 'empresa_id' => $this->empresaB->id]);
        $rolB->givePermissionTo($permisos);
        $this->userB->assignRole($rolB);
        $registrar->forgetCachedPermissions();

        $registrar->setPermissionsTeamId($this->empresaA->id);
        $this->clienteA = Cliente::create(['nombre' => 'Distribuidora Central', 'nit' => '900123456', 'empresa_id' => $this->empresaA->id]);
    }
    ...
    {
        Cliente::create(['nombre' => 'Otro Distinto', 'nit' => '111', 'empresa_id' => $this->empresaA->id]);
    ...
    {
        Cliente::create(['nombre' => 'Otro Cliente', 'email' => 'duplicado@test.com', 'empresa_id' => $this->empresaA->id]);
    ...
    {
        Cliente::create(['nombre' => 'Cliente Empresa A', 'email' => 'compartido@test.com', 'empresa_id' => $this->empresaA->id]);
    ...
    {
        Cliente::create(['nombre' => 'Cliente Sin NIT 1', 'empresa_id' => $this->empresaA->id]);
     ...
     * deshabilitar cruzado pero no /habilitar — mismo endpoint, mismo
     * mecanismo (resolverParaEmpresaActual), sin cobertura dedicada hasta
     * ahora (mismo gap ya cerrado en el cierre de Proveedores).
     */
    public function test_company_b_cannot_enable_company_as_client(): void
    {
        $this->actingAs($this->userA, 'api')
            ->postJson("/api/v1/clientes/{$this->clienteA->id}/deshabilitar");
        ...
        $this->assertSame('inactivo', $this->clienteA->fresh()->estado);
    }
