...
class MovimientoControllerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $rolA = Role::create(['name' => 'Test Movimientos A', 'guard_name' => 'api', 'empresa_id' => $this->empresaA->id]);
        $rolA->givePermissionTo($permisos);
        $this->userA->assignRole($rolA);
        $registrar->forgetCachedPermissions();

        $registrar->setPermissionsTeamId($this->empresaB->id);
        $rolB = Role::create(['name' => 'Test Movimientos B', 'guard_name' => 'api', 'empresa_id' => $this->empresaB->id]);
        $rolB->givePermissionTo($permisos);
        $this->userB->assignRole($rolB);
        $registrar->forgetCachedPermissions();

        $this->productoA = Producto::create(['codigo' => 'TEST-001', 'nombre' => 'Producto con movimientos', 'empresa_id' => $this->empresaA->id]);
    }
    ...
    {
        $proveedor = Proveedor::create(['nombre' => 'Proveedor Real', 'empresa_id' => $this->empresaA->id]);
     ...
     * unidad_medida_id de otra empresa aceptados sin verificar
     * pertenencia). Acá el controller ya usa resolverParaEmpresaActual()
     * para proveedor_id — nunca antes probado con un intento real de
     * usar un proveedor de otra empresa.
     */
    public function test_company_a_cannot_use_a_supplier_belonging_to_company_b_on_an_entrada_movement(): void
    {
        ...
        $this->assertSame(0.0, (float) $this->productoA->fresh()->stock_actual);
    ...
    {
        $this->productoA->forceFill(['stock_actual' => 100])->save();
        $proveedor = Proveedor::create(['nombre' => 'Proveedor X', 'empresa_id' => $this->empresaA->id]);
    ...
    public function test_filters_by_tipo_producto_and_search(): void
    {
        ...
        $otroProducto = Producto::create(['codigo' => 'OTRO-001', 'nombre' => 'Otro producto', 'empresa_id' => $this->empresaA->id]);
        $this->actingAs($this->userA, 'api')
    ...
    {
        $unidad = UnidadMedida::create(['nombre' => 'Kilogramo', 'abreviatura' => 'kg', 'empresa_id' => $this->empresaA->id]);
