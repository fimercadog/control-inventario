...
class ProductoProveedorControllerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $rol = Role::create(['name' => 'Test ProductoProveedor', 'guard_name' => 'api', 'empresa_id' => $this->empresaA->id]);
        ...
        $this->productoA = Producto::create(['categoria_id' => $categoria->id, 'codigo' => 'P-001', 'nombre' => 'Producto Test', 'empresa_id' => $this->empresaA->id]);
        $this->proveedorA1 = Proveedor::create(['nombre' => 'Distribuidora Uno', 'empresa_id' => $this->empresaA->id]);
        $this->proveedorA2 = Proveedor::create(['nombre' => 'Distribuidora Dos', 'empresa_id' => $this->empresaA->id]);
    }
    ...
    /**
     * Cierre de módulo (2026-08-11): mismo tipo de vulnerabilidad ya
     * encontrada y corregida en Productos (categoria_id/marca_id/
     * unidad_medida_id) y ya cerrada en Movimientos (proveedor_id en la
     * creación de un movimiento). Acá el controller ya usaba
     * resolverParaEmpresaActual() para proveedor_id — nunca antes probado
     * con un intento real de asociar el producto propio a un proveedor de
     * otra empresa. `test_company_b_cannot_view_update_or_disable_company_as_association`
     * prueba un escenario distinto (empresa B intentando tocar el
     * producto de empresa A por completo), no este.
     */
    public function test_company_a_cannot_associate_their_own_product_with_a_supplier_belonging_to_company_b(): void
    {
        $proveedorB = Proveedor::create(['nombre' => 'Proveedor de B', 'empresa_id' => $this->empresaB->id]);
        ...
        $this->assertDatabaseMissing('producto_proveedor', [
            'producto_id' => $this->productoA->id,
            'proveedor_id' => $proveedorB->id,
        ]);
    }
    ...
    public function test_an_association_must_belong_to_the_given_product(): void
    {
        ...
        $otroProducto = Producto::create(['categoria_id' => $categoria->id, 'codigo' => 'P-999', 'nombre' => 'Otro Producto', 'empresa_id' => $this->empresaA->id]);
        $asociacion = ProductoProveedor::create([
            'empresa_id' => $this->empresaA->id,
            'producto_id' => $this->productoA->id,
            'proveedor_id' => $this->proveedorA1->id,
