...
class ReporteProgramadoControllerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $rolGestor = Role::create(['name' => 'Test Reportes Gestor', 'guard_name' => 'api']);
        $rolGestor->givePermissionTo(['reportes.ver', 'reportes.gestionar']);
        $this->userGestor->assignRole($rolGestor);

        $rolSoloVer = Role::create(['name' => 'Test Reportes Solo Ver', 'guard_name' => 'api']);
    ...
    public function test_company_b_cannot_delete_company_as_scheduled_report(): void
    {
        ...
        $rolB = Role::create(['name' => 'Test Reportes B', 'guard_name' => 'api']);
        $rolB->givePermissionTo(['reportes.ver', 'reportes.gestionar']);
        $userB->assignRole($rolB);
        $registrar->forgetCachedPermissions();

        // `TenantScope` (BelongsToEmpresa) ya filtra la consulta de route
        // model binding por la empresa del usuario autenticado — el
        // registro de Empresa A ni siquiera se encuentra en el contexto
        // de Empresa B, así que el binding falla con 404 antes de que
