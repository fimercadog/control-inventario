...
class CompanyIsolationHttpTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // IdentifyEmpresa.php). La caché de permisos de Spatie guarda
        // permisos CON sus roles ya resueltos vía una query sobre
        // `App\Models\Role` — construir esa caché mientras el team activo
        // es A la deja "congelada" con solo los roles de A hasta que algo
        // la limpie. Lo que realmente aísla A de B en cada request real es
        // que `IdentifyEmpresa` llama `forgetCachedPermissions()` en cada
        // request, forzando una reconstrucción con el team correcto ANTES
        // de cualquier check. Aquí en `setUp()` no hay ninguna request
        // real todavía, así que se hace manualmente para dejar la caché
        // limpia después de armar ambos roles.
        $registrar->setPermissionsTeamId($this->empresaA->id);
        $rolA = Role::create(['name' => 'Test Captura IA A', 'guard_name' => 'api', 'empresa_id' => $this->empresaA->id]);
        $rolA->givePermissionTo($permisos);
        $this->userA->assignRole($rolA);
        $registrar->forgetCachedPermissions();

        $registrar->setPermissionsTeamId($this->empresaB->id);
        $rolB = Role::create(['name' => 'Test Captura IA B', 'guard_name' => 'api', 'empresa_id' => $this->empresaB->id]);
