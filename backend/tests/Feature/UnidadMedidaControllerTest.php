...
class UnidadMedidaControllerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $rol = Role::create(['name' => 'Test Unidades', 'guard_name' => 'api', 'empresa_id' => $this->empresaA->id]);
        ...
        $this->unidadA = UnidadMedida::create(['nombre' => 'Kilogramo', 'abreviatura' => 'kg', 'empresa_id' => $this->empresaA->id]);
    }
    ...
    /** `nombre` es el único campo obligatorio (`StoreUnidadMedidaRequest`) — encontrado sin cobertura auditando el módulo. */
    public function test_creating_a_unit_of_measure_without_a_name_is_rejected(): void
    {
        $this->actingAs($this->userA, 'api')
            ->postJson('/api/v1/unidades-medida', ['abreviatura' => 'sin-nombre'])
            ->assertStatus(422)
            ->assertJsonValidationErrors('nombre');
    }

    /** `estado` solo acepta `activo`/`inactivo` (`Rule::in`) — encontrado sin cobertura auditando el módulo. */
    public function test_creating_a_unit_of_measure_with_an_invalid_estado_is_rejected(): void
    {
        $this->actingAs($this->userA, 'api')
            ->postJson('/api/v1/unidades-medida', ['nombre' => 'Estado Inválido', 'estado' => 'archivada'])
            ->assertStatus(422)
    ...
    {
        UnidadMedida::create(['nombre' => 'Otra Distinta', 'abreviatura' => 'od', 'empresa_id' => $this->empresaA->id]);
     ...
     * `UpdateUnidadMedidaRequest` (antes aceptado con solo
     * `unidades-medida.editar`, más laxo que `unidades-medida.gestionar`,
     * el que exige `/deshabilitar`).
     */
    public function test_mass_assignment_of_identity_and_controlled_fields_is_rejected_in_a_single_request(): void
    {
        $original = $this->unidadA->fresh();
        $createdAtOriginal = $original->created_at;

        $this->actingAs($this->userA, 'api')
            ->patchJson("/api/v1/unidades-medida/{$this->unidadA->id}", [
                'id' => 999999,
        ...
        $actualizada = $this->unidadA->fresh();
        $this->assertSame($this->unidadA->id, $actualizada->id);
    ...
    {
        \App\Models\Producto::create(['unidad_medida_id' => $this->unidadA->id, 'codigo' => 'P-002', 'nombre' => 'Producto A', 'empresa_id' => $this->empresaA->id]);
        \App\Models\Producto::create(['unidad_medida_id' => $this->unidadA->id, 'codigo' => 'P-003', 'nombre' => 'Producto B', 'empresa_id' => $this->empresaA->id]);
     ...
     */
    public function test_company_b_cannot_enable_company_as_unit_of_measure(): void
    {
        $this->actingAs($this->userA, 'api')
            ->postJson("/api/v1/unidades-medida/{$this->unidadA->id}/deshabilitar");
