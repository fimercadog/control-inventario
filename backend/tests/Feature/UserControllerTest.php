...
class UserControllerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->roleConGestion = Role::create(['name' => 'Administrador', 'guard_name' => 'api', 'empresa_id' => $this->empresaA->id]);
        $this->roleConGestion->givePermissionTo('usuarios.editar', 'usuarios.ver');
        $this->roleAlterno = Role::create(['name' => 'Bodeguero', 'guard_name' => 'api', 'empresa_id' => $this->empresaA->id]);
        ...
        $this->roleEmpresaB = Role::create(['name' => 'Rol de Empresa B', 'guard_name' => 'api', 'empresa_id' => $this->empresaB->id]);
        // usuarios.ver/editar para que userB quede como "admin de su propia
        // empresa" — los tests cross-company de abajo deben aislar
        // exclusivamente el límite de empresa, no colarse con el de
        // permiso (que ya tiene su propia cobertura dedicada vía
        // $sinPermiso). Cierre de módulo (2026-08-11): antes de agregar
        // authorize('viewAny', ...) a index(), userB no necesitaba ningún
        // permiso para que estos tests pasaran.
        $this->roleEmpresaB->givePermissionTo('usuarios.ver', 'usuarios.editar');
        $this->userB->assignRole($this->roleEmpresaB);
        $registrar->forgetCachedPermissions();
    ...
    {
        $vendedor = Role::create(['name' => 'Vendedor', 'guard_name' => 'api', 'empresa_id' => $this->empresaA->id]);
    ...
    /**
     * Cierre de módulo (2026-08-11): `is_active`/`is_platform_admin`/
     * `empresa_id` están los tres en `User::$fillable` (a diferencia de
     * `name`/`email`, que además tienen su propio test dedicado) — la
     * única protección real es que `UpdateUsuarioRequest::rules()` nunca
     * los declara, así que `$request->validated()` los excluye siempre.
     * Nunca se había probado con un payload real que intentara los tres a
     * la vez, incluyendo el más crítico: escalar `is_platform_admin` a
     * `true` volvería a ese usuario invisible a todo `empresa_id` scoping
     * del resto del ERP.
     */
    public function test_privileged_fields_cannot_be_mass_assigned_via_the_generic_update_endpoint(): void
    {
        $colega = User::factory()->create(['empresa_id' => $this->empresaA->id, 'is_active' => true, 'is_platform_admin' => false]);
        ...
        $fresco = $colega->fresh();
        $this->assertTrue($fresco->is_active);
        $this->assertFalse($fresco->is_platform_admin);
    ...
    /**
     * Cierre de módulo (2026-08-11): a diferencia de `usuarios.editar` (ya
     * exigido por `UserPolicy::update()` desde la auditoría de campos
     * editables del 2026-08-04), listar/ver usuarios nunca exigió
     * `usuarios.ver` — ni `index()` invoca `authorize()`, ni
     * `UserPolicy::view()` verifica el permiso, solo pertenencia de
     * empresa. Cualquier usuario autenticado de la empresa (sin importar
     * su rol) podía enumerar el directorio completo de colegas (nombre,
     * email, rol, avatar, última actividad, IP). Confirmado con este test
     * ANTES de corregirlo (falla contra el código anterior); ver el fix en
     * `UserPolicy`/`UserController`.
     */
    public function test_a_user_without_usuarios_ver_cannot_list_or_view_colleagues(): void
    {
        $sinPermiso = User::factory()->create(['empresa_id' => $this->empresaA->id]);
        $colega = User::factory()->create(['empresa_id' => $this->empresaA->id]);
        ...
        $this->actingAs($sinPermiso, 'api')
            ->getJson("/api/v1/usuarios/{$colega->id}")
            ->assertStatus(403);
    }
