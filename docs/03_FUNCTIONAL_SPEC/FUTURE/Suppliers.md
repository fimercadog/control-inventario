...
> ⚠️ **Amendment (2026-08-04, auditoría de campos editables de Clientes/Proveedores/Usuarios).** Verificado contra código real (no contra este documento): `Proveedor` existe (`backend/app/Models/Proveedor.php`), la tabla `proveedores` existe, y `frontend/app/(app)/proveedores` es una pantalla real — 🟢 COMPLETE per `docs/03_FUNCTIONAL_SPEC/RC1_FUNCTIONAL_MODULE_AUDIT.md`. Campos reales: `nombre` (editable, ya no puede vaciarse en un PATCH desde esta fecha — `filled` en `UpdateProveedorRequest`), `nit`/`contacto`/`telefono`/`email`/`direccion`/`ciudad`/`pais`/`notas` (editables, `email` único por empresa, `nit` sin unicidad), `estado` (condicionalmente editable — solo vía `/habilitar`/`/deshabilitar` desde esta fecha, antes también aceptado por el PATCH genérico con el permiso más laxo `proveedores.editar` en vez de `proveedores.gestionar`), `empresa_id`/`id`/`created_at`/`updated_at` (read-only, estructural). Permisos reales: `proveedores.ver`/`proveedores.crear`/`proveedores.editar`/`proveedores.gestionar` sí existen y se aplican vía `ProveedorPolicy`. No hay Service/Repository dedicado — el Controller opera directo sobre el Model (`RC1_FUNCTIONAL_MODULE_AUDIT.md:51`).
>
> ⚠️ **Riesgo documentado, no corregido en esta auditoría** (nueva regla de negocio, no una ya decidida — fuera del mandato del rol Developer): `nit` no tiene constraint de unicidad, ni a nivel de base de datos ni de validación, ni por empresa ni global — dos proveedores con el mismo NIT en la misma empresa no se detectan. Añadirla requeriría antes revisar si ya existen duplicados reales en datos de producción.
