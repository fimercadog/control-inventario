...
>
> ⚠️ **Riesgos documentados, no corregidos en esta auditoría** (nueva regla de negocio, no una ya decidida — fuera del mandato del rol Developer): `nit` no tiene constraint de unicidad, ni a nivel de base de datos ni de validación, ni por empresa ni global — dos proveedores con el mismo NIT en la misma empresa no se detectan, y ahora tampoco es corregible después de crear el registro (`nit` es Identity desde ADR-015). Añadir unicidad requeriría antes revisar si ya existen duplicados reales en datos de producción. Tampoco existe ningún flujo de corrección para un typo real en `email`/`nit` al crear el registro — documentado en ADR-015, "Consecuencias negativas".
