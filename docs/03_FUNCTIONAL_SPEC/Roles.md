...
- **`/roles`** (`frontend/app/(app)/roles/page.tsx`): lista de roles de la empresa — búsqueda por nombre, filtro por estado, columnas Rol/Permisos/Usuarios/Estado, paginación real, acciones Editar/Desactivar-Activar. Botón "Nuevo Rol" abre `role-form-modal.tsx` (2026-08-03, reemplaza `new-role-dialog.tsx`). Único punto de entrada del módulo — no existe una ruta `/roles/{id}` (Global UI Standard "CRUD en Modal", 2026-08-03).
- **Ver/Editar** ocurren en `role-view-modal.tsx`/`role-form-modal.tsx` sobre este mismo listado (reemplazan `frontend/app/(app)/roles/[id]/page.tsx` → `role-detail-screen.tsx`, eliminados), nunca navegando a otra página — dos tabs dentro del modal de vista: "Detalle" (chips de permisos; "Editar" abre el modal de formulario con `PermissionPicker`) y "Usuarios" (lista de solo lectura de los usuarios asignados a ese rol, con email y estado activo/inactivo — existe explícitamente para que el mensaje 409 de "rol con usuarios asignados" sea accionable: el operador puede ver a quién tiene que reasignar).
