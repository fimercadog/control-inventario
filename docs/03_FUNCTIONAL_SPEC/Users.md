...
1. **Alcance original: Listar, Ver, Activar, Desactivar. Ampliado 2026-08-03 con Invitar y Asignar rol, y 2026-08-04 con Editar (`ADR-015`).** La creación de usuarios sigue sin ser un formulario directo — es responsabilidad exclusiva del Módulo 6 (Invitaciones, construido 2026-08-03): un admin invita por correo, el invitado elige su propia contraseña al aceptar. **Nombre/correo nunca son editables por nadie, ni siquiera el propio usuario** — son Identity (`ADR-015`, 2026-08-04; antes de esa fecha, nombre sí era autoservicio vía `Perfil`). Lo que sí se agregó 2026-08-04: un administrador con `usuarios.editar` puede editar los campos Operational (avatar/idioma/zona horaria/tema) de OTRO usuario de su empresa — decisión explícita del propietario del proyecto para mantener a Usuarios consistente con el patrón Ver/Editar/Controlado del resto del ERP, en vez de ser el único módulo sin ninguna acción "Editar". La reasignación de rol sí se construyó (2026-08-03, ahora que Módulo 5 — Roles — está completo): reemplaza el rol del usuario, nunca lo agrega a una lista — este ERP modela un único rol por usuario de punta a punta.
...
5. Un usuario con `usuarios.editar` puede activar o desactivar una cuenta ajena — nunca la propia (Decisión 2), y nunca si deja a la empresa sin nadie con `usuarios.editar` (Decisión 3) — puede reasignar el rol de cualquier usuario de su empresa (reemplaza el anterior, no lo acumula), y desde 2026-08-04 puede editar sus campos Operational (avatar, idioma, zona horaria, tema) vía el botón "Editar".
...
- **Usuario con `usuarios.editar`**: además puede activar/desactivar cuentas ajenas, reasignar su rol, y editar sus campos Operational (avatar/idioma/zona horaria/tema, desde 2026-08-04).
...
- **`/usuarios`**: tabla de usuarios de la empresa actual — nombre, email, rol, badge de estado, última actividad. Búsqueda por nombre/email, filtro de estado, filtro de rol, paginación real (100/página, Anterior/Siguiente). Botón "Nuevo Usuario" (2026-08-03) abre un diálogo de invitación (email + rol opcional) — no un formulario de creación directo. Menú de acciones de fila (2026-08-04, antes solo tenía Activar/Desactivar): Ver, Editar, Cambiar rol, Activar/Desactivar (con confirmación obligatoria) — mismo patrón directo-en-el-menú que el resto de los módulos CRUD, ya no hace falta abrir Ver primero para llegar a Cambiar rol. Único punto de entrada del módulo — no existe una ruta `/usuarios/{id}` (Global UI Standard "CRUD en Modal", 2026-08-03).
- **Ver** ocurre en un modal de solo lectura sobre este mismo listado (`UsuarioViewModal`), nunca navegando a otra página — nombre/correo siguen de solo lectura; el campo Rol gana una acción "Cambiar rol" (diálogo con selector, sin cambios); botones Editar y Activar/Desactivar en el header (mismo patrón que `ClienteViewModal`).
- **Editar** (nuevo, 2026-08-04, `ADR-015`) abre `UsuarioFormModal` — mismo patrón `CrudModal`+`Field` que el resto de los módulos CRUD. Muestra Nombre/Correo/Empresa/Platform Admin bloqueados (`Field locked`, ícono de candado + leyenda) y Avatar/Idioma/Zona horaria/Tema editables. `estado`/rol/contraseña no aparecen aquí — siguen en sus flujos dedicados (Activar-Desactivar, Cambiar Rol, y Cambiar Contraseña que sigue siendo autoservicio exclusivo, ningún admin puede tocar la contraseña de otro). El avatar se sube/quita de inmediato (como en Perfil), fuera del botón "Guardar cambios".
...
| --- | --- | --- |
| name, email, is_platform_admin | `users` | No — campos Identity (`ADR-015`), fijados una sola vez al aceptar la invitación, inmutables después, en ninguna ruta. `name` era editable vía `Perfil` hasta 2026-08-04; ADR-015 lo cerró. |
| empresa_id | `users` | No — Identity, fijo desde la creación (ver Decisión 5) |
| avatar_path, theme, language, timezone | `users` | Sí (desde 2026-08-04) — Operational, vía "Editar" (`UsuarioFormModal`), por un administrador con `usuarios.editar` sobre CUALQUIER usuario de su empresa. También editables por el propio usuario vía `Perfil` — dos rutas al mismo campo, ambas legítimas (ver `Profile.md`). |
...
**Clasificación (modelo de identidad ERP, `docs/08_ADR/ADR-015-identity-field-model.md`, 2026-08-04):** `name`/`email`/`empresa_id`/`is_platform_admin` son **Identity** — inmutables después de crearse la cuenta, sin excepción, en ninguna ruta, ni siquiera por el propio usuario (`email` es además la credencial de login, ver `Profile.md`). `is_active` y el rol asignado son **Controlled** — editables únicamente vía su endpoint dedicado, cada uno con su propio permiso (`is_active` además exige no dejar a la empresa sin nadie con `usuarios.editar`, Decisión 3; ver el riesgo documentado arriba sobre `asignarRol()`, que no replica esa segunda guarda). `avatar_path`/`theme`/`language`/`timezone` son **Operational** — editables por el propio usuario vía `Perfil`, y desde 2026-08-04 también por un administrador con `usuarios.editar` sobre otro usuario de su empresa vía "Editar" (reversión deliberada del invariante anterior "nadie edita el perfil de otro", ver `Profile.md` y `ADR-015`). `password` es Controlled (Cambiar Contraseña) — self-service exclusivo, ningún administrador puede cambiar la contraseña de otro usuario, sin excepción — ver `Profile.md`.
...
- [x] (2026-08-04) Un administrador con `usuarios.editar` puede editar avatar/idioma/zona horaria/tema de otro usuario de su empresa; nombre/correo nunca se aceptan, ni siquiera enviados explícitamente (`ADR-015`).
