...
**Estado real del código, actualizado en Fase 4.5 (2026-08-02) — el resto de este documento (§1-3, §6-11) sigue tal cual quedó en Módulo 2 y no refleja los módulos construidos desde Fase 1 del roadmap RC1; ver `docs/security/ROLES_MATRIX.md` para el estado exhaustivo y actual.** De las 12 Policies que existen hoy, **8 ya verifican un permiso Spatie real** (`AND`eado con la pertenencia de empresa, nunca en su lugar): `CategoriaPolicy`, `MarcaPolicy`, `UnidadMedidaPolicy`, `StockPolicy`, `ProveedorPolicy`, `ProductoProveedorPolicy`, `UserPolicy` (más la guarda de negocio del último administrador). Quedan sin ese segundo factor, deliberadamente fuera del alcance de Fase 4.5: `ProductoPolicy`, `MovimientoPolicy`, `CapturaIAPolicy` — mismo gap que tenían los ocho anteriores hasta esta fase, documentado como decisión pendiente en `ROLES_MATRIX.md`. Lo que sigue faltando, y sigue siendo Módulo 3 (Authorization/RBAC, `[ ]` en `docs/00_VISION/Roadmap.md`): middleware que aplique un permiso a nivel de ruta (hoy el chequeo vive en la Policy, evaluado siempre, pero sin esa capa adicional), y el `PermissionContext`/sidebar dinámico del frontend. `spatie/laravel-permission` con Teams (`team_foreign_key = empresa_id`) está instalado, `Role` subclasificado con `BelongsToEmpresa` (`Models/Role.php`, con columna `estado` agregada en Fase 4.5), catálogo de permisos sembrado (`PermissionSeeder`, 40 permisos). Ver `docs/08_ADR/ADR-010-rbac-teams.md`.
...
| RBAC con permisos reales en Policies | `$user->can()` en toda acción | 8/12 Policies lo hacen desde Fase 4.5 (2026-08-02); `ProductoPolicy`/`MovimientoPolicy`/`CapturaIAPolicy` siguen solo tenant-ownership — ver `ROLES_MATRIX.md`. Middleware de ruta (Módulo 3) sigue pendiente en cualquier caso. |
