...
**Estado real del código, actualizado en Fase 4.6 (2026-08-02) — el resto de este documento (§1-3, §6-11) sigue tal cual quedó en Módulo 2 y no refleja los módulos construidos desde Fase 1 del roadmap RC1; ver `docs/security/ROLES_MATRIX.md` para el estado exhaustivo y actual.** De las 10 Policies que existen hoy, **9 ya verifican un permiso Spatie real** (`AND`eado con la pertenencia de empresa, nunca en su lugar): `CategoriaPolicy`, `MarcaPolicy`, `UnidadMedidaPolicy`, `StockPolicy`, `ProveedorPolicy`, `ProductoProveedorPolicy` (Fase 4.5) + `ProductoPolicy`, `MovimientoPolicy`, `CapturaIAPolicy` (Fase 4.6, cerró el gap que estas tres tenían deliberadamente pendiente desde Fase 4.5). Queda exactamente una excepción deliberada y anterior a ambas fases: `UserPolicy` (RC1 Fase 4) solo verifica pertenencia de empresa, más la guarda de negocio del último administrador — no un permiso Spatie. Lo que sigue faltando, y sigue siendo Módulo 3 (Authorization/RBAC, `[ ]` en `docs/00_VISION/Roadmap.md`): middleware que aplique un permiso a nivel de ruta (hoy el chequeo vive en la Policy, evaluado siempre, pero sin esa capa adicional), y el `PermissionContext`/sidebar dinámico del frontend. `spatie/laravel-permission` con Teams (`team_foreign_key = empresa_id`) está instalado, `Role` subclasificado con `BelongsToEmpresa` (`Models/Role.php`, con columna `estado` agregada en Fase 4.5), catálogo de permisos sembrado (`PermissionSeeder`, 41 permisos). Ver `docs/08_ADR/ADR-010-rbac-teams.md`.
...
| RBAC con permisos reales en Policies | `$user->can()` en toda acción | 9/10 Policies lo hacen desde Fase 4.6 (2026-08-02, cerró el gap que dejó Fase 4.5 en Productos/Movimientos/Captura IA); solo `UserPolicy` sigue solo tenant-ownership, deliberado desde RC1 Fase 4 — ver `ROLES_MATRIX.md`. Middleware de ruta (Módulo 3) sigue pendiente en cualquier caso. |
| Auditoría genérica multi-módulo | Todo evento importante | Captura IA + cada módulo construido desde Fase 1 (Categorías/Marcas/Unidades de Medida/Stock/Movimientos/Usuarios/Proveedores) ya escriben en `AuditLog` vía `AuditLogger` |
