...
## Flujo "Editar" en Usuarios — consistencia de UX con el resto del ERP

Tras cerrar el modelo de campos arriba, Usuarios quedó como el único módulo del ERP sin ninguna acción "Editar" — coherente con la clasificación de campos (ningún campo Operational le pertenecía a un administrador sobre OTRO usuario, todos eran autoservicio vía Perfil), pero inconsistente con la UX del resto de los 7 módulos CRUD, cuyo menú de acciones siempre expone Ver/Editar/Deshabilitar directamente, nunca requiere abrir un modal primero para descubrir una acción.

**Alternativas consideradas** (debate explícito con el propietario del proyecto):
1. **No agregar Editar; solo hacer consistente el menú de acciones** (agregar "Ver"/"Cambiar rol" como items directos del dropdown, ya reachable antes solo abriendo el modal). Propuesta inicial — resuelve la inconsistencia del menú, pero no la inconsistencia de patrón ("todo módulo CRUD tiene Editar").
2. **Renombrar "Ver" a "Administrar" y construir un panel de administración nuevo.** Descartada — el panel ya existe (`UsuarioViewModal` ya muestra Rol/Estado/Actividad/Trazabilidad todo junto); renombrarlo habría hecho a Usuarios el único módulo del ERP cuyo modal de detalle no se llama "Ver", cambiando una inconsistencia por otra.
3. **Agregar "Editar" real, exponiendo los campos Operational de otro usuario a un administrador — elegida.** Decisión explícita del propietario del proyecto, como arquitecto del proyecto: "la consistencia pesa más que el hecho de que haya pocos campos editables." Mantiene exactamente el mismo patrón Ver→Editar→Controlado (Rol/Estado) que ya usan Clientes/Proveedores/etc., sin inventar un concepto de UX nuevo.

**Decisión**: `UserController` gana `actualizar()` (`PATCH /usuarios/{id}`, campos Operational — `theme`/`language`/`timezone`), `subirAvatar()`/`eliminarAvatar()` (`POST`/`DELETE /usuarios/{id}/avatar`) — las tres autorizan vía la misma ability `update` de `UserPolicy` (`usuarios.editar`) ya usada por `activar`/`desactivar`/`asignarRol`. `UsuarioFormModal` (nuevo, sigue el patrón `CrudModal`+`Field` de los demás módulos) muestra Nombre/Correo/Empresa/Platform Admin bloqueados (`Field locked`) y Avatar/Idioma/Zona horaria/Tema editables — `estado`/rol/contraseña no aparecen en absoluto, siguen en sus flujos dedicados. El menú de acciones de cada fila gana Ver/Editar/Cambiar rol directos (antes solo Desactivar); `UsuarioViewModal` gana su propio botón "Editar" (mismo patrón que `ClienteViewModal`).

**Reversión deliberada de una regla anterior**: `docs/03_FUNCTIONAL_SPEC/Profile.md` documentaba "Ningún usuario puede editar el perfil de otro — estructuralmente imposible, no solo rechazado" como criterio de aceptación cumplido (`ProfileController` nunca aceptó un `{id}` en su ruta, por diseño). Esa invariante ahora es falsa para avatar/idioma/zona horaria/tema — sigue siendo verdadera para nombre/correo (Identity) y contraseña (Controlled, autoservicio exclusivo). `ProfileService`/`ProfileController` no se tocaron — la nueva capacidad vive enteramente en `UserController`, un controller distinto con su propia autorización (`usuarios.editar`), no una relajación del invariante de `ProfileService`.

## Impacto
Medio-Alto — cambia comportamiento de API ya en producción (tres endpoints de actualización dejan de aceptar campos que antes aceptaban, dos endpoints de creación ganan una validación de unicidad nueva, tres endpoints nuevos permiten a un administrador editar campos antes exclusivos de autoservicio) y UI de tres formularios más un módulo completo (Usuarios gana Editar); agrega un constraint de base de datos (`nit` único por empresa) pero no cambia el modelo de permisos existente (reutiliza `usuarios.editar` ya existente). La regla "Identity Correction" no tiene impacto todavía — es una decisión arquitectónica registrada, sin código.

## Referencias
- `docs/03_FUNCTIONAL_SPEC/Customers.md` (sección "Editable Fields")
- `docs/03_FUNCTIONAL_SPEC/Users.md`
- `docs/03_FUNCTIONAL_SPEC/Profile.md` (nota de reversión del invariante "nadie edita el perfil de otro")
- `docs/03_FUNCTIONAL_SPEC/FUTURE/Suppliers.md` (amendment)
- `docs/11_DESIGN_SYSTEM/FORMS.md` (regla de afordancia visual `locked`)
- `backend/app/Http/Requests/Cliente/{Store,Update}ClienteRequest.php`, `Proveedor/{Store,Update}ProveedorRequest.php`, `Profile/UpdateProfileRequest.php`, `User/UpdateUsuarioRequest.php`
- `backend/database/migrations/2026_08_04_140000_add_unique_nit_to_clientes_and_proveedores.php`
- `backend/app/Http/Controllers/Api/UserController.php` (`actualizar`/`subirAvatar`/`eliminarAvatar`), `backend/app/Http/Resources/User/UserResource.php`, `backend/routes/api.php`
- `frontend/components/crud-modal.tsx` (`Field` con prop `locked`), `cliente-form-modal.tsx`, `proveedor-form-modal.tsx`, `usuario-form-modal.tsx` (nuevo), `usuario-view-modal.tsx`, `asignar-rol-dialog.tsx` (control externo), `frontend/app/(app)/perfil/page.tsx`, `frontend/app/(app)/usuarios/page.tsx`, `frontend/lib/api/client.ts` (fix del mensaje de error específico por campo), `frontend/lib/user-preferences.ts` (nuevo)
...
**Modelo de campos (Identity/Operational/Controlled), unicidad de `nit`, y flujo "Editar" en Usuarios: implementados.** Backend: 3 `UpdateXRequest` modificados + 1 nuevo (`UpdateUsuarioRequest`), `StoreClienteRequest`/`StoreProveedorRequest` con `nit` único por empresa, 1 migración nueva, 3 endpoints nuevos en `UserController` (`PATCH {id}`, `POST`/`DELETE {id}/avatar`), suite completa **405/405** (era 391 antes de esta ronda de cambios). Frontend: `Field` compartido gana `locked`, aplicado a 5 campos identity-lock + 4 en `UsuarioFormModal` (nuevo); menú de acciones de Usuarios gana Ver/Editar/Cambiar rol directos; `npx tsc --noEmit` limpio; verificado en navegador real de punta a punta.
