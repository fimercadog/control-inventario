...
**Regla hacia adelante — campos de identidad (`docs/08_ADR/ADR-015-identity-field-model.md`, 2026-08-04):** todo formulario de edición nuevo debe clasificar sus campos como Identity/Operational/Controlled antes de decidir qué inputs son editables. Un input Identity en modo edición se renderiza siempre (el usuario debe poder ver el valor), pero con `disabled` — nunca oculto. `disabled` solo no basta como afordancia — `Field` (`frontend/components/crud-modal.tsx`) acepta un prop `locked?: boolean` que agrega un ícono de candado junto a la etiqueta y una leyenda ("Campo de identidad. No se puede modificar después de crear el registro.") debajo del input, para que la razón sea obvia sin que el usuario tenga que intentar escribir primero. Aplicado en `ClienteFormModal`/`ProveedorFormModal` (NIT, Email) y en `/perfil` (Nombre, Correo — este último migrado de `<Label>`+`<Input>` suelto al `Field` compartido para heredar la afordancia). Cualquier campo Identity nuevo debe usar `Field locked`, no reinventar el candado/leyenda localmente.
