...
**"Módulo que lo consume hoy" = ¿algún `Policy`/`Controller` llama `$user->can('ese.permiso')` o equivalente?** Tras Fase 4.6 + los módulos Clientes, Roles, Auditoría y Reportes (ampliado 2026-08-03 con `ReportePolicy`), **13 de las 14 Policies existentes** (`ProductoPolicy`, `MovimientoPolicy`, `CapturaIAPolicy`, `CategoriaPolicy`, `MarcaPolicy`, `UnidadMedidaPolicy`, `StockPolicy`, `ProveedorPolicy`, `ProductoProveedorPolicy`, `ClientePolicy`, `RolePolicy`, `AuditLogPolicy`, `ReportePolicy`) verifican un permiso Spatie real, **AND**eado con la pertenencia de empresa preexistente — nunca reemplazándola. Queda exactamente **una** excepción deliberada y anterior a Fase 4.5/4.6: `UserPolicy` (RC1 Fase 4) solo verifica pertenencia de empresa — `usuarios.ver` no tiene ningún consumidor todavía (ver fila arriba); la única lógica de negocio real hoy sobre Usuarios es la guarda del último administrador (`esElUltimoConGestion()`), no un chequeo de permiso Spatie. `usuarios.invitar` tampoco tiene Policy propia todavía — espera al Módulo 6 (Invitaciones), no es un gap del modelo de autorización en sí. **`reportes.ver` sigue siendo un caso aparte, incluso después de la ampliación**: las acciones de lectura/generación/exportación de reportes (`ReporteController::index/catalogo/preview/exportar*/historial`) siguen sin un Model Eloquent natural al cual atar una Policy (agregan sobre varios modelos distintos, no gestionan uno propio), así que `$this->authorize('reportes.ver')` sigue resolviendo directo contra el `Gate::before()` global que Spatie ya registra para cualquier nombre de permiso. Lo que sí cambió: `reportes_programados`/`reporte_historial` **sí** son modelos Eloquent propios (nuevos en la ampliación), así que `reportes.gestionar` (crear/eliminar un reporte programado) queda enforced vía la nueva `ReportePolicy` — una sola clase de Policy para los dos modelos, registrada explícitamente en `AppServiceProvider::boot()` porque el auto-discovery de Laravel espera una Policy por modelo. Ver `docs/05_IMPLEMENTATION/ReportesModule.md`.
