...
`PermissionSeeder` (guard `api`, global y fijo — ninguna empresa lo edita), 46 permisos sembrados hoy:
...
| `reportes.ver` | Ver estadísticas de Productos/Inventario/Movimientos/Clientes/Proveedores | **Sí** — sin Policy propia, `$this->authorize('reportes.ver')` directo en `ReporteController` (2026-08-02, ver sección 3) |
...
**"Módulo que lo consume hoy" = ¿algún `Policy`/`Controller` llama `$user->can('ese.permiso')` o equivalente?** Tras Fase 4.6 + los módulos Clientes, Roles, Auditoría y Reportes, **12 de las 13 Policies existentes** (`ProductoPolicy`, `MovimientoPolicy`, `CapturaIAPolicy`, `CategoriaPolicy`, `MarcaPolicy`, `UnidadMedidaPolicy`, `StockPolicy`, `ProveedorPolicy`, `ProductoProveedorPolicy`, `ClientePolicy`, `RolePolicy`, `AuditLogPolicy`) verifican un permiso Spatie real, **AND**eado con la pertenencia de empresa preexistente — nunca reemplazándola. Queda exactamente **una** excepción deliberada y anterior a Fase 4.5/4.6: `UserPolicy` (RC1 Fase 4) solo verifica pertenencia de empresa — `usuarios.ver` no tiene ningún consumidor todavía (ver fila arriba); la única lógica de negocio real hoy sobre Usuarios es la guarda del último administrador (`esElUltimoConGestion()`), no un chequeo de permiso Spatie. `usuarios.invitar` tampoco tiene Policy propia todavía — espera al Módulo 6 (Invitaciones), no es un gap del modelo de autorización en sí. **`reportes.ver` es un caso aparte**: está enforced (`ReporteController::index()` llama `$this->authorize('reportes.ver')`), pero sin una 14ª Policy — "Reportes" no tiene un Model Eloquent natural al cual atar una clase de Policy (agrega sobre 5 modelos distintos, no gestiona uno propio), así que el ability se resuelve directo contra el `Gate::before()` global que Spatie ya registra para cualquier nombre de permiso (`PermissionRegistrar::registerPermissions()`) — el mismo mecanismo subyacente que usa cada Policy internamente, solo que invocado sin una clase intermedia. Ver `docs/05_IMPLEMENTATION/ReportesModule.md`.
...
| Reportes | Sin modelo propio — agrega sobre `Producto`/`Movimiento`/`Cliente`/`Proveedor`/`ProductoProveedor` | Automático (heredado de los 5 modelos fuente, ya con `TenantScope`) | **Sin Policy propia, a propósito** — `$this->authorize('reportes.ver')` directo (2026-08-02, ver Gap 9) |
...
| **Reportes** | ✔ (un único resumen compuesto, no una lista) | — (no aplica, no hay un "registro" individual) | — (nunca) | — (nunca) | — (nunca aplica) | **—** (nunca, no hay nada que eliminar) | `reportes.ver` — **enforced (2026-08-02)**; único permiso, sin Policy propia (ver sección 3) |
...
| `reportes.ver` | Módulo Reportes — **enforced (2026-08-02)**, sin Policy propia (ver sección 3, Gap 9) |
