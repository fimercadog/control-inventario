...
const EMPTY_DASHBOARD: DashboardSummary = {
  ...
  crm: { contactos: 0, prospectos: 0, oportunidades_abiertas: 0, valor_pipeline: 0, actividades_pendientes: 0, actividades_vencidas: 0, actividades_vencidas_destacadas: [], oportunidades_destacadas: [], proximas_actividades: [] },
};
    ...
    <div className="mx-auto flex w-full max-w-[1160px] flex-col gap-6 pb-4">
      ...
      <section className="grid gap-5 lg:grid-cols-[minmax(0,1.8fr)_minmax(18rem,0.9fr)]">
        ...
        <div className="flex flex-col gap-4">
          ...
          {canViewActivities && dashboard.crm.actividades_vencidas > 0 ? <Card className="rounded-3xl border-destructive/40 bg-destructive-container py-0"><CardContent className="p-4"><div className="mb-3 flex items-center gap-2 text-destructive-container-foreground"><TriangleAlert className="size-4" /><h2 className="font-semibold">Seguimientos vencidos</h2></div><p className="mb-3 text-sm text-destructive-container-foreground/80">{numberFormat.format(dashboard.crm.actividades_vencidas)} actividad{dashboard.crm.actividades_vencidas === 1 ? " requiere" : "es requieren"} atención.</p><ul className="flex flex-col gap-2">{dashboard.crm.actividades_vencidas_destacadas.map((activity) => <li key={activity.id} className="min-w-0 text-sm"><p className="truncate font-medium text-foreground">{activity.asunto}</p><p className="truncate text-xs text-destructive-container-foreground/80">{activity.cliente ?? "Sin cliente"}{activity.oportunidad ? ` · ${activity.oportunidad}` : ""}</p></li>)}</ul><Link href="/actividades" className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-destructive-container-foreground hover:underline">Ver seguimientos <ArrowRight className="size-4" /></Link></CardContent></Card> : null}
