...
function MetricCard({ icon: Icon, label, value, tone }: { icon: typeof Package; label: string; value: number; tone: string }) {
  return (
    <Card className="rounded-3xl border border-border bg-card py-0 shadow-none">
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1160px] flex-col gap-6 pb-4">
      <section className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        ...
        {canUseCapture ? <Button className="h-10 rounded-xl bg-primary px-4 text-primary-foreground shadow-sm hover:bg-primary/90" nativeButton={false} render={<Link href="/captura-ia" />}><Sparkles className="size-4" />Nueva captura</Button> : null}
      </section>
      ...
      <section className="grid gap-5 lg:grid-cols-[minmax(0,1.8fr)_minmax(18rem,0.9fr)]">
        <Card className="min-h-[32rem] rounded-3xl border border-border py-0 shadow-none">
          <CardContent className="p-4 sm:p-5">
            ...
            {dashboard.movimientos_recientes.length === 0 ? <p className="py-16 text-center text-sm text-muted-foreground">No hay movimientos recientes.</p> : <ul className="divide-y divide-border">{dashboard.movimientos_recientes.map((movement) => { const isEntry = movement.delta >= 0; return <li key={movement.id} className="flex items-center gap-3 py-3"><span className="grid size-9 shrink-0 place-items-center rounded-full bg-muted text-muted-foreground"><Package className="size-4" /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-foreground">{movement.producto ?? "Producto"}</p><p className="truncate text-xs text-muted-foreground">{formatDateTime(movement.created_at)}{movement.usuario ? ` · ${movement.usuario}` : ""}</p></div><div className="text-right"><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${isEntry ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}>{isEntry ? "↙ Entrada" : "↗ Salida"}</span><p className="mt-1 text-xs text-muted-foreground">{movement.delta > 0 ? "+" : ""}{numberFormat.format(movement.delta)}</p></div></li>; })}</ul>}
          </CardContent>
        </Card>

        <div className="flex flex-col gap-4">
          {canUseCapture ? <Card className="rounded-3xl border-0 bg-primary py-0 text-primary-foreground shadow-none"><CardContent className="p-5"><span className="mb-5 grid size-10 place-items-center rounded-2xl bg-white/15"><Sparkles className="size-5" /></span><h2 className="font-semibold">Registra inventario en segundos</h2><p className="mt-1 text-sm text-indigo-100">Toma una foto, habla, o ambos — nosotros actualizamos el stock.</p><Button className="mt-5 w-full justify-between rounded-xl bg-white text-primary hover:bg-indigo-50" nativeButton={false} render={<Link href="/captura-ia" />}>Ir a Captura IA <ArrowRight className="size-4" /></Button></CardContent></Card> : null}
          <Card className="rounded-3xl border border-border py-0 shadow-none"><CardContent className="p-4"><h2 className="mb-4 font-semibold text-foreground">Acciones rápidas</h2><div className="flex flex-col gap-2">{canUseCapture ? <><Button variant="outline" className="h-11 justify-start rounded-xl" nativeButton={false} render={<Link href="/captura-ia" />}><Camera className="size-4" />Capturar por foto</Button><Button variant="outline" className="h-11 justify-start rounded-xl" nativeButton={false} render={<Link href="/captura-ia" />}><Mic className="size-4" />Capturar por voz</Button></> : null}{canViewProducts ? <Button variant="outline" className="h-11 justify-start rounded-xl" nativeButton={false} render={<Link href="/productos" />}><Package className="size-4" />Ver productos</Button> : null}</div></CardContent></Card>
          <Card className="rounded-3xl border border-amber-200 bg-amber-50 py-0 shadow-none"><CardContent className="p-4"><div className="mb-3 flex items-center gap-2 text-amber-800"><TriangleAlert className="size-4" /><h2 className="font-semibold">Stock bajo</h2></div>{dashboard.productos_con_stock_bajo.length === 0 ? <p className="text-sm text-amber-800/70">No hay productos bajo el mínimo.</p> : <ul className="flex flex-col gap-2">{dashboard.productos_con_stock_bajo.slice(0, 4).map((product) => <li key={product.id} className="flex items-center justify-between gap-3 text-sm"><span className="truncate text-foreground">{product.nombre}</span><span className="shrink-0 font-medium text-amber-700">{numberFormat.format(product.stock_actual)} {product.unidad_medida ?? ""}</span></li>)}</ul>}</CardContent></Card>
        </div>
      </section>
    </div>
  );
